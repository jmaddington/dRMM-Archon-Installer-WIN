...
$archon_token = Get-dRMMVariable -site_variable $env:site_archon_deployment_token -script_variable $env:runtime_archon_deployment_token -exit_if_missing $true -exit_message "ERROR! Missing deployment token, unable to continue"

Write-Host "Installing with token $archon_token"

.\ZorusInstaller.exe /qn ARCHON_TOKEN=$archon_token

#Exit with success, to exit with failure use $exitcode = 1
$exitcode = 0

Exit-dRMMScript -exitcode $exitcode -results "Success!" -diagnostics "The Zorus installer didn't report any errors. This does NOT mean it installed correctly."